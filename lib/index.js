...
ctx.ReactOnRails = {
  ...
  registerStore: function registerStore(stores) {
    if (!stores) {
      throw new Error('Called SnabbdomOnRails.registerStores with a null or undefined, rather than ' + 'an Object with keys being the store names and the values are the store generators.');
    }
  ...
  setOptions: function setOptions(newOptions) {
    ...
    if (Object.keys(newOptions).length > 0) {
      throw new Error('Invalid options passed to SnabbdomOnRails.options: ', JSON.stringify(newOptions));
    }
  },
